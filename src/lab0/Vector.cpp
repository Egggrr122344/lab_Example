...
Vector<T>::Vector(const Vector<T>& vec) {
    ...
    try {
        uninitialized_copy(vec.array, vec.array + vec.size, new_arr);
    } catch (...) {
...
template <typename T>
Vector<T>::Vector(const initializer_list<T>& lst) {
    T* new_arr = reinterpret_cast<T*>(new int8_t[lst.size() * sizeof(T)]);
    try {
        uninitialized_copy(lst.begin(), lst.end(), new_arr);
    } catch (...) {
        delete[] reinterpret_cast<char*>(new_arr);
        throw;
    }
...
void Vector<T>::reserve(size_t n) {
    ...
    try {
        uninitialized_copy(array, array + size, newArr);
    } catch (...) {
